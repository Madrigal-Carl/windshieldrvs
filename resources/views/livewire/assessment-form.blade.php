                            ...
                            <div class="mt-2 flex w-full text-xs text-gray-600">
                                @foreach ($sectionBars as $seg)
                                    <div style="flex: {{ $seg['weight'] }}; text-align: center; min-width:0;">
                                        <div class="{{ $seg['textColor'] }} text-[0.5rem] md:text-base">
...
<script>
    ...
    async function saveAllSectionsToServer($wire) {
        ...
        for (let i = 0; i < sections.length; i++) {
            ...
            // Force desktop (md and up) styling
            clone.style.fontFamily = "'Poppins', sans-serif";
            clone.style.width = "794px"; // A4 width
            clone.style.fontSize = "1rem"; // match md:text-base
            clone.style.lineHeight = "1.5";
            clone.classList.add('md'); // fake the presence of md breakpoint

            // Override all text sizes to md equivalents
            clone.querySelectorAll('.text-xs').forEach(el => el.style.fontSize = '0.875rem');
            clone.querySelectorAll('.text-sm').forEach(el => el.style.fontSize = '1rem');
            clone.querySelectorAll('.text-[\\.6rem]').forEach(el => el.style.fontSize = '0.875rem');

            // Adjust layout classes for md versions
            clone.querySelectorAll('.w-full.md\\:w-2\\/3').forEach(el => el.style.width = '66.666667%');
            clone.querySelectorAll('.w-full.md\\:w-1\\/3').forEach(el => el.style.width = '33.333333%');

            // Inject consistent font
